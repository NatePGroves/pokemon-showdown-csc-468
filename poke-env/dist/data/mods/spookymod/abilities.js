...
const Abilities = {
  ...
  magician: {
    ...
    onFaint(pokemon) {
      const faint = [
        ...
        "I die, I diieeee... bye Soldier.",
        "I die! Soldier, you were the wooorst roommate!",
  ...
  powerofalchemy: {
    ...
    onStart(pokemon) {
      ...
      const possibleTargets = pokemon.foes().filter((foeActive) => foeActive && !foeActive.getAbility().flags["cantsuppress"] && !additionalBannedAbilities.includes(foeActive.ability) && foeActive.isAdjacent(pokemon));
  ...
  pulpup: {
    ...
    onModifyType(move, pokemon) {
      ...
      if (move.type === "Normal" && !noModifyType.includes(move.id) && !(move.isZ && move.category !== "Status") && move.name !== "Explosion" && !(move.name === "Tera Blast" && pokemon.terastallized)) {
        move.type = "Fire";
